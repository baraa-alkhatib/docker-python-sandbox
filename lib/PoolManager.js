...
const DEFAULT_OPTIONS = {
  containerLaunchOptions: {
    "Image": "professphysics/docker-sandbox",
...
class PoolManager {
  ...
  async initialize(size, cb) {
    ...
    for (let i = 0; i < containers.length; i += 1) {
      ...
      if (containerInfo.Image === 'professphysics/docker-sandbox') {
        await this.docker.getContainer(containerInfo.Id).kill();
      }
    }
  ...
  _executeJob(job, cb) {
    ...
    this.tempPortsHolder.push({ hostPort: container.hostPort });
    log.debug("[pool] Executing a new job", {'executor': container.id })

    const retryOptions = {
      times: 10, 
      interval: Number(job.payload.timeoutMs) || 500 // TODO: accommodate for long time processing
    }
    ...
    async.waterfall([
      ...
      /*
       * Job execution is over. We call the user callback BEFORE
       * cleaning up and replacing the container with a fresh one.
       */
      (result, next) => {
        log.debug('[pool] job done')
        job.cb(null, result)
        ...
        container.cleanup(() => _.remove(this.tempPortsHolder, { hostPort: container.hostPort }));
  ...
  _initializeContainer (cb) {
     ...
     * or macOS host, one way around this is to fallback to mapping ports
    */
   var hostPort = null;
    if (process.platform === 'darwin' || process.platform === 'win32') {
      hostPort = 3200;
      if (this.availableContainers.length + this.bootingContainers.length + this.tempPortsHolder.length) {

        var hostPortsObjArr = [...this.availableContainers, ...this.bootingContainers, ...this.tempPortsHolder];
        if (hostPortsObjArr.length) {
          hostPortsObjArr.map(o => Number(o.hostPort)).sort().forEach(h => {
            if (h === hostPort) {
              hostPort = h + 1;
            }
          });
      ...
      this.options.containerLaunchOptions.HostConfig.PortBindings = {
        '3000/tcp': [
          {
      ...
      this.tempPortsHolder.push({ 
        hostPort: hostPort
      })
    }

    this.docker.createContainer(this.options.containerLaunchOptions, (err, instance) => {
      // remove hostPort from the tempPortsHolder array
      _.remove(this.tempPortsHolder, { hostPort: hostPort });


      if (err) return cb(err)
      const container = new Container(uuid(), instance);

      // assign host port to the new container if found
      if (hostPort) {
        container.setHostPort(hostPort.toString());
      }
